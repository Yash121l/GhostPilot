...
function TrendsScreen({ state, set }) {
  ...
  const doRefresh = () => {
    ...
    setTimeout(() => {
      setRefreshing(false)
      set.toast('Refreshed — 3 new signals')
    }, 900)
  }
  ...
  return (
    <div className="fade-in">
      ...
      >
        <button
          className={`btn ${showConfig ? 'primary' : ''}`}
      ...
      {showConfig && (
        <div className="card fade-in" style={{ padding: 22, marginBottom: 18 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 18 }}>
            ...
            <span className="chip linkedin on" style={{ fontSize: 10.5 }}>
          ...
          <label className="label" style={{ marginTop: 16 }}>
            ...
            <span className="mono" style={{ float: 'right', color: 'var(--text-3)' }}>
              {threshold}%
            </span>
          </label>
          <input
          ...
          <div style={{ display: 'flex', gap: 10, marginTop: 18 }}>
            ...
            <button
              ...
              onClick={() => {
                setKeywords([])
                setThreshold(40)
                setSources({ hackernews: true, reddit: true })
              }}
            >
              Reset defaults
            </button>
...
function TrendCard({ trend, onDraft }) {
  ...
  return (
    ...
    >
      <div>
        <div style={{ fontSize: 11, letterSpacing: '0.14em', fontWeight: 700, color: tagColor }}>
          {trend.tag}
        </div>
        <div
          style={{
...
function ScoreBar({ label, value, color }) {
  return (
    <div>
      ...
      >
        <span>{label}</span>
        <span className="mono" style={{ color: 'var(--text-2)', fontWeight: 500 }}>
          {value}
        </span>
      </div>
...
function PersonasScreen({ state, set }) {
  ...
  const save = () => {
    ...
    setSelected(p.id)
    setCreating(false)
    setDraft({ name: '', bio: '', pillars: '', style: '' })
      ...
      <div style={{ overflow: 'auto', padding: 28 }}>
        {creating ? (
          <PersonaForm
            ...
            onCancel={() => {
              setCreating(false)
              setSelected(state.personas[0]?.id)
            }}
          />
        ) : persona ? (
          <PersonaDetail
  ...
  return (
    <div style={{ maxWidth: 580 }}>
      ...
      <input
        className="input"
        value={draft.pillars}
        onChange={(e) => setDraft({ ...draft, pillars: e.target.value })}
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 22 }}>
        <button className="btn primary" onClick={onSave}>
          <IconSave size={14} /> Create Persona
        </button>
...
function PersonaField({ label, value, render }) {
  return (
    <div>
      ...
      <div style={{ fontSize: 14, lineHeight: 1.6 }}>{render ? render(value) : value}</div>
    </div>
  )
}
  ...
  return (
    <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
        ...
        <Kpi
          label="Impressions"
          value={(a.impressions / 1000).toFixed(1) + 'K'}
          delta="+38.2%"
          positive
        />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 16 }}>
        ...
        <div className="card" style={{ padding: 20 }}>
          ...
          >
            <div>
              ...
              >
                Impressions · last 14d
              </div>
              <div style={{ fontSize: 24, fontWeight: 700, marginTop: 4 }}>
                {(a.impressions / 1000).toFixed(1)}K
              </div>
            </div>
            <div style={{ display: 'flex', gap: 6 }}>
              {['7d', '14d', '30d', '90d'].map((r) => (
                <button
                  ...
                  className="chip"
                  style={{
                    background: r === '14d' ? 'var(--accent-soft)' : undefined,
                    color: r === '14d' ? 'var(--accent)' : undefined,
                    borderColor: r === '14d' ? 'rgba(91,91,240,0.18)' : undefined
                  }}
                >
                  {r}
                </button>
        ...
        <div className="card" style={{ padding: 20 }}>
          ...
          {a.byPlatform.map((p) => {
            const Ico =
              p.id === 'linkedin' ? IconLinkedIn : p.id === 'twitter' ? IconAt : IconInstagram
            return (
              <div key={p.id} style={{ marginBottom: 14 }}>
                ...
                <div className="progress" style={{ height: 6 }}>
                  <div
                    className="fill"
                    style={{ width: (p.eng / 10) * 100 + '%', background: p.color }}
                  />
                </div>
          ...
          {a.topPosts.map((p, i) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className="mono"
                  style={{ fontSize: 12, color: 'var(--text-3)', width: 70, textAlign: 'right' }}
                >
                  {p.reach.toLocaleString()}
                </div>
                <div
                  className="mono"
                  style={{ fontSize: 12, color: 'var(--success)', width: 50, textAlign: 'right' }}
                >
                  {p.eng}%
                </div>
              </div>
...
function Kpi({ label, value, delta, positive }) {
  return (
    <div className="card" style={{ padding: 18 }}>
      ...
      <div
        style={{ fontSize: 12, marginTop: 4, color: positive ? 'var(--success)' : 'var(--text-3)' }}
      >
        {delta}
      </div>
    </div>
  ...
  const addKey = () => {
    ...
    set.apiKeys([
      ...state.apiKeys,
      {
        id: 'k' + Date.now(),
        provider,
        label: label || providers[provider].name,
