...
function ComposerScreen({ state, set }) {
  ...
  return (
    <div className="fade-in" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, height: '100%' }}>
      {/* DRAFT PANEL */}
      <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 12 }}>
          <div style={{ fontSize: 11, letterSpacing: '0.14em', textTransform: 'uppercase',
            color: 'var(--text-3)', fontWeight: 600 }}>Platforms</div>
        ...
        <div className="card" style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
          <div style={{ display: 'flex', gap: 4, padding: '8px 10px', borderBottom: '1px solid var(--border)' }}>
            {toolbarItems.map(t => {
              ...
              return (
                <button key={t.id} className="btn ghost" style={{ padding: '6px 10px', fontSize: 13 }}
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between',
            padding: '8px 14px', borderTop: '1px solid var(--border)', background: 'var(--bg-subtle)',
            color: 'var(--text-3)', fontSize: 12 }}>
            <div style={{ display: 'flex', gap: 14 }}>
              <span>📎 0 attachments</span>
              <span>Persona: <strong style={{ color: 'var(--text-2)' }}>Yash — Indie Founder</strong></span>
            </div>
...
function VariantEmpty() {
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column',
      alignItems: 'center', justifyContent: 'center', textAlign: 'center', gap: 14, color: 'var(--text-3)' }}>
...
function VariantCard({ variant, platform, onSchedule, onCopy, onPublish }) {
  const content = platform === 'twitter'
    ? variant.thread.map((t, i) => (
        <div key={i} style={{ padding: '10px 0', borderBottom: i < variant.thread.length - 1 ? '1px dashed var(--border)' : 'none' }}>
          <div style={{ display: 'flex', gap: 10 }}>
            <div className="mono" style={{ color: 'var(--text-4)', fontSize: 12, paddingTop: 2 }}>{i + 1}</div>
  ...
  return (
    <div className="card fade-in" style={{ padding: 20 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <div style={{ fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--text-3)', fontWeight: 600 }}>
          Variant · {platform}
        </div>
      ...
      <div style={{ display: 'flex', gap: 8, paddingTop: 12, borderTop: '1px solid var(--border)' }}>
        ...
        <button className="btn" onClick={onSchedule}><IconClock size={14} />Schedule</button>
        <div style={{ flex: 1 }} />
        <button className="btn primary" onClick={onPublish}><IconSend size={14} />Publish now</button>
      </div>
    </div>
  );
}
...
function CalendarScreen({ state, set }) {
  ...
  return (
    <div className="fade-in" style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: 24, height: '100%' }}>
      {/* DAY DETAIL / TIMELINE */}
      <div style={{ display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        ...
        <div className="card" style={{ flex: 1, overflow: 'auto', padding: 0 }}>
          {postsForDay(selectedDay).length === 0 ? (
            <div style={{ padding: '60px 20px', textAlign: 'center', color: 'var(--text-3)' }}>
              ...
              <div style={{ color: 'var(--text)', fontSize: 16, fontWeight: 600 }}>Nothing scheduled</div>
              <div style={{ marginTop: 4 }}>Open Composer to draft and schedule a post.</div>
              <button className="btn primary" style={{ marginTop: 18 }}
                onClick={() => set.active('composer')}>Open Composer</button>
            </div>
          ) : (
            <div style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 10 }}>
              {postsForDay(selectedDay).sort((a,b) => a.hour - b.hour).map(p => (
                <SchedulePostRow key={p.id} post={p} onDelete={() => {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div className="card" style={{ padding: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 4 }}>
              <button className="btn ghost icon" style={{ width: 26, height: 26 }}><IconChevron size={14} style={{ transform: 'rotate(180deg)' }} /></button>
              <button className="btn ghost icon" style={{ width: 26, height: 26 }}><IconChevron size={14} /></button>
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7,1fr)', gap: 4, marginBottom: 6 }}>
            {['Su','Mo','Tu','We','Th','Fr','Sa'].map(d =>
              <div key={d} style={{ fontSize: 10, color: 'var(--text-3)', textAlign: 'center', fontWeight: 600, letterSpacing: '0.04em' }}>{d}</div>
            )}
          </div>
            ...
            {cells.map((d, i) => {
              ...
              return (
                <button key={i}
                  ...
                  {d}
                  {has > 0 && !isSel && (
                    <div style={{ position: 'absolute', bottom: 3, width: 4, height: 4, borderRadius: '50%',
        ...
        <div className="card" style={{ padding: 14 }}>
          ...
          {[
            ...
            const Ico = r.id === 'linkedin' ? IconLinkedIn : r.id === 'twitter' ? IconAt : IconInstagram;
            ...
            return (
              <div key={r.id} style={{ marginBottom: 10 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 4 }}>
                  ...
                  <span className="mono" style={{ color: 'var(--text-3)' }}>{r.val}/{r.cap}</span>
                </div>
                <div className="progress"><div className="fill" style={{ width: (pct*100)+'%', background: r.color }} /></div>
              </div>
            );
          })}
        </div>
        ...
        <div className="card" style={{ padding: 14 }}>
          <div style={{ fontSize: 11, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--text-3)', fontWeight: 600, marginBottom: 10 }}>
            This Week
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13 }}>
            <div>
              <div style={{ fontSize: 22, fontWeight: 700 }}>{state.scheduled.filter(p => p.status === 'scheduled').length}</div>
              <div style={{ color: 'var(--text-3)', fontSize: 12 }}>Scheduled</div>
            </div>
            <div>
              <div style={{ fontSize: 22, fontWeight: 700 }}>{state.scheduled.filter(p => p.status === 'draft').length}</div>
...
function SchedulePostRow({ post, onDelete }) {
  ...
  const bgMap = { linkedin: 'var(--linkedin-soft)', twitter: 'var(--twitter-soft)', instagram: 'var(--instagram-soft)' };
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '10px 12px',
      ...
      <div style={{ width: 32, height: 32, borderRadius: 8, background: bgMap[post.platform],
        color: colorMap[post.platform], display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <Ico size={15} />
      </div>
      ...
      <button className="btn ghost icon" onClick={onDelete}><IconTrash size={14} /></button>
    </div>
  );
}
...
function ConnectScreen({ state, set }) {
  const items = [
    { id: 'linkedin',  name: 'LinkedIn',    desc: 'Professional posts, articles, and thought leadership', icon: 'IconLinkedIn' },
    { id: 'twitter',   name: 'X (Twitter)', desc: 'Tweets, threads, and real-time engagement',           icon: 'IconAt' },
    { id: 'instagram', name: 'Instagram',   desc: 'Visual content, carousels, and caption publishing',    icon: 'IconInstagram' },
  ];
  ...
  return (
    <div className="fade-in" style={{ maxWidth: 780 }}>
        ...
        {items.map(p => {
          ...
          return (
            <div key={p.id} className="card" style={{ display: 'flex', alignItems: 'center', gap: 16, padding: 16 }}>
              <div style={{ width: 44, height: 44, borderRadius: 10, background: bgMap[p.id],
                color: colorMap[p.id], display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Ico size={20} />
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  {connected ? (
                    <span style={{ fontSize: 11, color: 'var(--success)', display: 'inline-flex', alignItems: 'center', gap: 4, fontWeight: 500 }}>
              ...
              {connected ? (
                <button className="btn danger" onClick={() => set.connections({ ...state.connections, [p.id]: false })}>Disconnect</button>
              ) : (
      ...
      <div style={{ marginTop: 28, fontSize: 11, letterSpacing: '0.14em', textTransform: 'uppercase',
        color: 'var(--text-3)', fontWeight: 600, marginBottom: 10, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
        <IconShield size={12} /> Privacy &amp; Security
      </div>
      <div className="card" style={{ padding: 18, fontSize: 13.5, color: 'var(--text-2)', lineHeight: 1.7 }}>
        <p style={{ margin: '0 0 10px' }}>
          OAuth tokens are stored exclusively in your OS keychain (macOS Keychain / Windows Credential
          Manager / libsecret on Linux). They are <strong>never uploaded</strong> to any server or stored in the database.
        </p>
        <p style={{ margin: 0 }}>
          All publishing requests originate directly from this machine. Disconnecting immediately deletes the local token.
        </p>
      </div>
    </div>
  );
      ...
      <div className="fade-in" style={{ maxWidth: 640 }}>
        <div className="card" style={{ padding: 24 }}>
          ...
          <input className="input" value={northStar} onChange={e => setNorthStar(e.target.value)}
            placeholder="e.g. Reach 10,000 LinkedIn followers by Q4" autoFocus />
          <label className="label" style={{ marginTop: 16 }}>Timeframe</label>
          <select className="select" defaultValue="6 months">
            <option>4 weeks</option><option>12 weeks</option><option>6 months</option><option>1 year</option>
          </select>
          <label className="label" style={{ marginTop: 16 }}>Primary platform</label>
          <select className="select">
            <option>LinkedIn</option><option>X (Twitter)</option><option>Instagram</option><option>All platforms</option>
          </select>
          <div style={{ display: 'flex', gap: 10, marginTop: 22 }}>
            <button className="btn primary" onClick={() => {
              if (!northStar.trim()) return;
              const g = {
                id: 'g'+Date.now(), name: northStar, north: northStar,
                progress: 0.05, current: 124, target: 5000, weekly: 4,
                keyResults: [
                  { label: 'Publish 4 LinkedIn posts / week', done: false, current: 1, target: 4 },
              ...
              setCreating(false); setNorthStar('');
              set.toast('Goal created. AI generated 3 key results.');
            }}><IconSparkle size={14} /> Decompose with AI</button>
  ...
  return (
    <div className="fade-in" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: -8 }}>
        <button className="btn primary" onClick={() => setCreating(true)}><IconPlus size={14} /> New Goal</button>
      </div>
      {state.goals.map(g => <GoalCard key={g.id} goal={g} set={set} />)}
    </div>
  );
}
    ...
    <div className="card" style={{ padding: 22 }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 16 }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 18, fontWeight: 700, marginTop: 4, letterSpacing: '-0.01em' }}>{goal.name}</div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div className="mono" style={{ fontSize: 22, fontWeight: 700 }}>{goal.current.toLocaleString()}</div>
          <div style={{ fontSize: 12, color: 'var(--text-3)' }}>of {goal.target.toLocaleString()}</div>
        </div>
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {goal.keyResults.map((kr, i) => (
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <div style={{ width: 18, height: 18, borderRadius: 4, border: '1.5px solid ' + (kr.done ? 'var(--success)' : 'var(--border-strong)'),
                background: kr.done ? 'var(--success)' : 'transparent',
                display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff' }}>
                {kr.done && <IconCheck size={11} />}
              </div>
              <div style={{ flex: 1, fontSize: 14, color: kr.done ? 'var(--text-3)' : 'var(--text)',
                textDecoration: kr.done ? 'line-through' : 'none' }}>{kr.label}</div>
