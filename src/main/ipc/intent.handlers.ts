...
export function registerIntentHandlers(): void {
  ipcMain.handle(
    ...
    async (_event, { personaId }: { personaId?: string }) => {
      try {
        ...
        return err(
          e instanceof AppError
            ? e
            : new AppError({ code: ErrorCode.DB_QUERY_FAILED, message: String(e) })
        )
      }
    }
  )
  ...
  ipcMain.handle(
    ...
    async (_event, input: Omit<Intent, 'id' | 'createdAt' | 'updatedAt' | 'keyResults'>) => {
      try {
  ...
  ipcMain.handle(
    ...
    async (_event, input: Partial<Intent> & { id: string }) => {
      try {
        const { id, ...rest } = input
